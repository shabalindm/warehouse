...
class Model5 extends MultyItemsModel {
	...
	public Model5(Connection conn){
		try {
			this.conn = conn;		
			daos = new DAO [3];
			daos[0] = new DAO(conn, "ÊÎÌÏËÅÊÒÓŞÙÈÅ", "ÊÎÌÏË_ID");
			daos[1] = new DAO(conn, "ÊÎÌÏËÅÊÒÓŞÙÈÅ", "ÊÎÌÏË_ID");
			daos[2] = new DAO(conn, "ÒĞÅÁÎÂÀÍÈß", "ÒĞÅÁ_ID");
			columnsMap = new int[][]{
					
					{2,  0},//ÒĞÅÁ_ID     NOT NULL NUMBER         
					{2,  1},//ÍÎÌ_ÒĞÅÁ    NOT NULL VARCHAR2(40)   
					{2,  2},//ÑÏÅÖ_ID     NOT NULL NUMBER         
					{2,  3},//ÍÎÌ_ÇÀßÂÊÈ           VARCHAR2(40)   
					{2,  4},//ÇÀÏĞÎØÅÍÎ            NUMBER(7,3)    
					{2,  5},//ÂÛÄÀÍÎ               NUMBER(7,3)  
					{2,  6},//ID_ÇÀÏĞÎØ            NUMBER 
					     
					//{0,  0},//ÊÎÌÏË_ID     NOT NULL NUMBER        
					{0,  1},//ÃĞÓÏÏÀ                VARCHAR2(100) 
					{0,  2},//ÍÀÈÌÅÍÎÂÀÍÈÅ          VARCHAR2(200) 
					{0,  3},//ÒÓ                    VARCHAR2(100) 
					{0,  4},//ÌÀĞÊÀ                 VARCHAR2(100) 
					{0,  5},//ÅÄ_ÈÇÌÅĞÅÍÈß          VARCHAR2(40)  
					{0,  6},//ÍÀ_ÑÊËÀÄÅ             NUMBER(7,3)   
					{0,  7},//ÊÎÌÏ_ÈÍÔÎ             VARCHAR2(400) 
					//{0,  8},//ÄÀÒÀ_ÊÎÌÏ             DATE  
					
					{2,  7},//ID_ÂÛÄÀÍ    NOT NULL NUMBER  
					...
					{2,  8},//ÄÀÒÀ_ÒĞÅÁ            DATE           
					{2,  9},//ÄÀÒÀ_ÂÛÄÀ×È          DATE           
					{2,  10},//ÈÍÔÎ                 VARCHAR2(1000) 
	...
	public void find(int rowIndex){
		...
		String sqlFind;
			...
			if(items[2].getVal(6) instanceof BigDecimal) {//items[1].getVal(6) -  ıòî ID çàïğîøåííîãî 
				sqlFind = "SELECT * FROM ÊÎÌÏËÅÊÒÓŞÙÈÅ WHERE ÊÎÌÏË_ID = " + items[2].getVal(6);
			} else{
			...
			try{
				...
				if (rs.next()){
					items[2].setVal(6, rs.getObject("ÊÎÌÏË_ID"));
			...
			if(items[2].getVal(7) instanceof BigDecimal) {//items[1].getVal(7) -  ıòî ID âûäàííîãî
				sqlFind = "SELECT * FROM ÊÎÌÏËÅÊÒÓŞÙÈÅ WHERE ÊÎÌÏË_ID = " + items[2].getVal(7);
			} else{
				String tu = (items[0].getVal(3) == null ) ? " is null " : " = '" + items[0].getVal(3).toString().trim().toUpperCase() + "'";
				String mark = (items[0].getVal(4) == null ) ? " is null " : " = '" + items[0].getVal(4).toString().trim().toUpperCase() + "'";		
				sqlFind = "SELECT * FROM ÊÎÌÏËÅÊÒÓŞÙÈÅ WHERE ÒÓ " + tu + " AND ÌÀĞÊÀ " + mark;
			...
			try{
				...
				ResultSet rs = stmt.executeQuery(sqlFind);
				if (rs.next()){
					items[2].setVal(7, rs.getObject("ÊÎÌÏË_ID"));
	...
	Item[] getRowFromResultSet(int rowNumber) {
		...
		try {
			...
			row[0].pollFieldsFromResultSet(rs, "ID_ÇÀÏĞÎØ Â_ÃĞÓÏÏÀ Â_ÍÀÈÌÅÍÎÂÀÍÈÅ Â_ÒÓ Â_ÌÀĞÊÀ Â_ÅÄ_ÈÇÌÅĞÅÍÈß  Â_ÍÀ_ÑÊËÀÄÅ Â_ÊÎÌÏ_ÈÍÔÎ Â_ÄÀÒÀ_ÊÎÌÏ".split("\\s+"));
